...
const Hero = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await axios.get(`http://localhost:5000/api/tiktok/download?url=${url}`);
      if (res.data.success) {
  ...
  const handleDownload = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/tiktok/stream?url=${encodeURIComponent(targetUrl)}`);
